...
export function ProxyContextProvider({contextClass, children, value, onChange, proxyRef}) {
    ...
    const set = useCallback((newObj) => {
        if (newObj !== objRef.current) {
            ...
            if (!isNullOrUndefined(newObj)) {
                ...
                objRef.current = new Proxy(newObj, {
                    ...
                    set: function(target, prop, value) {
                        ...
                        Reflect.set(target, prop, value, objRef.current);
                        value = target[prop];
                        ...
                        Object.values(changeSubscribers.current).forEach(subscriber => {
                            if (
                                    ...
                                    ( 
                                        ...
                                        ||
                                    subscriber.deps.some(subProp => isCallable(subProp) && subProp(prop, value, prevValue, objRef.current))
                            ) {
            ...
            // Chained conditional setter syntax (proxyRef?.current = objRef.current) is experimental. This is the alternative.
